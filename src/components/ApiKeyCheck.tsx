...
export function ApiKeyCheck({ children }: ApiKeyCheckProps) {
  ...
  useEffect(() => {
    // Check API key only on client side to avoid hydration mismatch
    setIsApiKeyMissing(!process.env.NEXT_PUBLIC_TAMBO_API_KEY);
  }, []);

  // Return loading state during hydration to avoid mismatch
      ...
      <div className="flex items-start gap-4">
        <div className="flex-grow">
          <div className="flex items-center gap-1">
            ...
            <p>Checking Tambo initialization...</p>
          </div>
        </div>
      </div>
    );
