...
export const McpConfigModal = ({
  ...
  React.useEffect(() => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem("mcp-servers") ?? "[]");
      setMcpServers(saved);
    } catch {
      // Ignore parse errors
    }
  ...
  React.useEffect(() => {
    // Skip initial render to avoid overwriting localStorage before we've loaded
...
export function useMcpServers(): McpServer[] {
  ...
  // Load servers from localStorage after mount
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    try {
      const loadedServers = JSON.parse(savedServersData);
      // Deduplicate servers by URL to prevent multiple tool registrations
      const uniqueUrls = new Set();
      const deduped = loadedServers.filter((server: McpServer) => {
